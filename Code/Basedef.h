...
struct STRUCT_MOB
{
	...
	unsigned char  Resist[4];	 // The mob's resistencies, to fire / ice / thunder / magic

	unsigned short Jewel; // nova var add para a joia da precisão
	short EMPY; // var vazia 1
	short EMPY2; // var vazia2 / limite atingido
...
struct TP_HEADER
{
	...
	TP_HEADER()
	{
		memset(this, 0, sizeof(MSG_HEADER));
	}

	TP_HEADER(short Type, short ID) //: this->Size = sizeof(MSG_STANDARD_TEMPLATE), this->ID = ID, this->Type = Type;
	{
		memset(this, 0, sizeof(TP_HEADER));
		this->Size = sizeof(TP_HEADER);
		this->ID = ID;
		this->Type = Type;
	}
	TP_HEADER(short Type, short ID, T Data)
	{
		memset(this, 0, sizeof(TP_HEADER));
		this->Size = sizeof(TP_HEADER);
		this->ID = ID;
...
{
	_MSG;
	char String[126]; // tamanho 128 original, diminuido 2 bytes para adicionar algo pro switch
	short Color;
};
